...
async function checkDependencies() {
  const check = (cmd, args = ['--version']) => new Promise(resolve => {
    try {
      ...
      console.error(`[Diagnostic] Failed to check ${cmd}:`, e.message);
      resolve(false);
    }
  });

  return {
    ytdlp: await check(getToolPath('yt-dlp'), ['--version']),
    ffmpeg: await check(getToolPath('ffmpeg'), ['-version'])
  };
}
