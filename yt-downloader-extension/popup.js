    ...
    if (res.ok) {
      ...
      if (data.dependencies && (!data.dependencies.ytdlp || !data.dependencies.ffmpeg)) {
        ...
        if (currentVideo && !isDownloading) {
          btnLabel.innerHTML = `Install ${missing} <span style="font-size:10px; opacity:0.8;">(Required)</span>`;
          downloadBtn.disabled = false;
          downloadBtn.onclick = () => {
            const url = missing === 'yt-dlp' ? 'https://github.com/yt-dlp/yt-dlp#installation' : 'https://ffmpeg.org/download.html';
  ...
  try {
    ...
    async function attemptDownload() {
      ...
      try {
        ...
        console.warn('[Engineer] POST failed, retrying with GET...', postErr);
